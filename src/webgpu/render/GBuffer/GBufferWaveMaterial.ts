...
export default class GBufferWaveMaterial extends Material {
    ...
    setup() {
        ...
        this.addAttribute("colors", ShaderType.vec4, 1, VertexStepMode.Instance);
...
let model= mat4x4<f32>(instancesMatrix0,instancesMatrix1,instancesMatrix2,instancesMatrix3);
   ...
   /* let normalMatrix= mat3x3<f32>(
    ...
    output.color =colors.xyz;
    output.normal =normalize(aNormal);// normalize(normalMatrix*aNormal);
   // output.uv =aUV0;
    return output;
}
...
fn mainFragment(${this.getFragmentInput()}) ->  GBufferOutput
{
    var output : GBufferOutput;
     ...
     output.color =vec4(color.x,color.x,color.x,1.0);
