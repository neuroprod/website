...
export default class EditCamera {
    ...
    checkMouse() {
        ...
        if (!UI.needsMouse() && this.mouseListener.wheelDelta) {
            ...
            this.camDistance += this.mouseListener.wheelDelta / 20;
        ...
        if (this.mouseListener.isUpThisFrame && this.isDragging) {
            ...
            AppState.setState("camQuat", this.camQuat)
            AppState.setState("camDistance", this.camDistance)
            AppState.setState("camTarget", this.camTarget)
        }
    ...
    setActive() {
        if (AppState.getState("camQuat")) {
            this.camQuat.from(AppState.getState("camQuat"))
            this.camDistance = AppState.getState("camDistance")
            this.camTarget.from(AppState.getState("camTarget"))
            this.setCamera()
        }
    }
